...
export default function PractiseMockTest({
  params,
}: {
  params: { slug: string };
}) {
  ...
  const greenBackground = "bg-green-500";
  const redBackground = "bg-red-500";
  ...
  const isOptionSelectedAndWrong = (optionText: string) => {
    ...
    return (
      selectedOptions[currentQuestion] === optionText &&
                  ...
                  (option, optionIndex) => (
                    <ul key={optionIndex} className="mb-2">
                      <label className="flex items-center">
                        ...
                        <span
                          className={`ml-1 ${
                            isOptionSelectedAndWrong(option.text)
                              ? "bg-red-500" // Green background for selected and wrong option
                              : selectedOptions[currentQuestion] != null &&
                                option.isCorrect
                              ? "bg-green-500" // Green background for correct option
                              : isOptionSelectedAndWrong(option.text)
                              ? "bg-red-500" // Red background for selected and wrong option
                              : ""
                          }`}
                        >
                          {option?.text}
